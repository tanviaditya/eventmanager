...
public class LoginAttendeeFrame extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					LoginAttendeeFrame frame = new LoginAttendeeFrame();
					frame.pack();
	...
	public LoginAttendeeFrame() {
		...
		JLabel lblLogin = new JLabel("ATTENDEE LOGIN");
		...
		JLabel lblId = new JLabel("ATTENDEE ID  :");
		...
		JLabel lblPassword = new JLabel("YOUR NAME :");
	...
	void authenticateUser() throws IOException {
		...
		if(m.AtendeeLogin(textField_1.getText(), textField.getText())) {
			JOptionPane.showMessageDialog(null, "Login Successfull", "Login", 1);
			setVisible(false);
			AttendeeFrame f = new AttendeeFrame();
			f.setVisible(true);
		} else {
